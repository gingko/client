...
toDag posets =
    let
        ...
        getPrev pos =
            ...
            in
            posets
                |> List.filter (\p -> p.next == Just pos.uid)
                -- List Tree
                |> List.map .uid
                -- List String
                |> List.append prevId
        ...
        getNext pos =
            ...
            in
            posets
                |> List.filter (\p -> p.prev == Just pos.uid)
                -- List Tree
                |> List.map .uid
                -- List String
                |> List.append nextId
        ...
        f : Tree -> DagEntry
        f p =
            DagEntry p.uid (getPrev p) (getNext p)
    in
    posets
        -- List { uid: String, prev: Maybe String, next: Maybe String }
        |> List.map f
...
sortFunction dag a b =
    let
        distA =
            maxDist dag "end" a.uid

        distB =
            maxDist dag "end" b.uid
    in
    if distA == distB then
        compare a.uid b.uid
    ...
    if toId == fromId then
        ...
        let
            maxNexts =
                dag
                    -- List DagEntry
                    |> ListExtra.find (\de -> de.uid == fromId)
                    -- Maybe DagEntry
                    |> Maybe.withDefault (DagEntry "end" [ "start" ] [])
                    -- DagEntry
                    |> .next
...
linearizeDag dag =
    dag
        -- List DagEntry
        |> List.sortWith (sortFunction dag)
        -- List DagEntry
        |> List.map .uid
